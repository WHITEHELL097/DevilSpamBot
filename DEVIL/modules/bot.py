...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        start = datetime.now()
        devil = await e.reply(f"» __𝙳𝙴𝚅𝙸𝙻__", parse_mode=None, link_preview=None)
        end = datetime.now()
        mp = (end - start).microseconds / 1000
        await devil.edit(f"__😈 ᴘɪɴɢ__\n» `{mp} ms`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ok = await event.reply(f"» __𝙱𝙴𝚃𝙴 𝙺𝙸 𝙻𝙸𝚂𝚃 𝙼𝙰𝙸 𝙰𝙳𝙳 𝙺𝚁𝚁 𝚁𝙷𝙰 𝙷𝚄😈...__")
        mks = "SUDO_USER"
        target = ""
        ...
        if len(sudousers) > 0:
            ...
            newsudo = f"{target}"
        await ok.edit(f"» **ɴᴇᴡ 𝙱𝙴𝚃𝙰 𝙵𝙾𝚄𝙽𝙳**: `{target}`\n» `ʀᴇsᴛᴀʀᴛɪɴɢ 💔😈...`")
